...
const App: React.FC = () => {
  ...
  const twistRotation = chaosFactor * 15; // Increased max rotation for better tie-dye feel
  ...
  const saturationBoost = savageEffect * 60; // Savage makes things more "intense/bloody"
  const brightnessBoost = witEffect * 20;   // Wit makes things "clearer/brighter"
  const hueShift = chaosFactor * 25;        // Chaos warps the colors
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden flex items-center justify-center">
        {/* Heat (Red Glow) */}
        <div 
          className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(220,38,38,0.15),transparent)] transition-opacity duration-700"
          style={{ opacity: savageEffect }}
        ></div>
        {/* Intellect (Blue Glow) */}
        <div 
          className="absolute inset-0 bg-[radial-gradient(circle_at_80%_80%,rgba(59,130,246,0.15),transparent)] transition-opacity duration-700"
          style={{ opacity: witEffect }}
        ></div>
        
        {/* Tie-Dye Chaos Twists */}
        <div 
          className="absolute w-[200%] aspect-square bg-[radial-gradient(circle_at_center,rgba(255,0,255,0.06),transparent_40%)] transition-transform duration-1000 ease-out"
          style={{ transform: `rotate(${twistRotation * 2.5}deg)` }}
        ></div>
        <div 
          className="absolute w-[200%] aspect-square bg-[radial-gradient(circle_at_center,rgba(0,255,255,0.03),transparent_60%)] transition-transform duration-1000 ease-out delay-100"
      ...
      <main 
        className="max-w-7xl w-full px-4 md:px-16 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mt-16 relative z-10 transition-transform duration-1000 ease-out"
        style={{ 
      ...
      >
        ...
        {/* Left: Input Lab */}
        <section className="glass-panel p-8 md:p-10 rounded-[3rem] space-y-8 h-fit border-t-4 border-t-orange-600 transition-all duration-700 shadow-2xl shadow-black relative overflow-hidden">
          ...
          <div className="space-y-8 relative z-10">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-black/60 border border-white/5 p-4 rounded-2xl focus:outline-none focus:border-orange-500/30 transition-all text-xl font-bold placeholder:text-gray-800"
                  value={settings.targetName}
                  onChange={(e) => setSettings({ ...settings, targetName: e.target.value })}
                />
                <textarea
                  placeholder="Describe their failure..."
                  className="w-full bg-black/60 border border-white/5 p-4 rounded-2xl focus:outline-none focus:border-orange-500/30 transition-all min-h-[80px] text-xs leading-relaxed"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <h2 className="text-sm font-bangers tracking-[0.2em] text-orange-500/80 uppercase">ROAST STYLE</h2>
                <select 
                  className="w-full bg-black/60 border border-white/5 p-3 rounded-xl focus:outline-none focus:border-orange-500/30 text-xs font-bold appearance-none cursor-pointer hover:bg-black/80 transition-colors"
              ...
              <div className="space-y-3">
                <h2 className="text-sm font-bangers tracking-[0.2em] text-orange-500/80 uppercase">PRIMARY FOCUS</h2>
                <select 
                  className="w-full bg-black/60 border border-white/5 p-3 rounded-xl focus:outline-none focus:border-orange-500/30 text-xs font-bold appearance-none cursor-pointer hover:bg-black/80 transition-colors"
            ...
            <div className="space-y-8">
              ...
              <div className="bg-black/60 rounded-[2.5rem] p-6 border border-white/10 flex flex-col items-center shadow-inner">
              ...
              <div className="space-y-8 bg-black/30 p-8 rounded-[2.5rem] border border-white/5 shadow-inner">
                <Slider
                  label="HEAT LEVEL"
                  min={0}
                  max={100}
          ...
          <button
            ...
            disabled={loading}
            className={`w-full py-6 rounded-[2rem] text-3xl font-bangers tracking-[0.2em] transition-all duration-500 transform hover:-translate-y-2 active:scale-95 shadow-2xl ${
              loading 
        ...
        {/* Right: Result Zone */}
        <section className="flex flex-col gap-10 min-h-[500px]" ref={resultRef}>
          {currentRoast ? (
            <div key={currentRoast.id} className="glass-panel p-8 md:p-12 rounded-[4rem] relative overflow-hidden transition-all duration-1000 animate-[burnIn_0.8s_cubic-bezier(0.22,1,0.36,1)] border-white/10 shadow-3xl">
              ...
              <div className="space-y-12">
                ...
                <div className="relative py-8 text-center">
                   <blockquote className="relative text-2xl md:text-4xl font-bangers tracking-tight leading-[1.2] text-white drop-shadow-2xl px-2">
                    "{currentRoast.text}"
                  </blockquote>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center bg-black/60 p-8 rounded-[3.5rem] border border-white/10 shadow-inner">
                  ...
                  <div className="flex flex-col gap-4">
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 p-5 rounded-2xl transition-all border border-white/5 group"
                    >
                    ...
                    {currentRoast.caricatureUrl && (
                      <a 
                        ...
                        className="w-full flex items-center justify-center gap-3 p-5 rounded-2xl transition-all border border-white/5 bg-white/5 hover:bg-white/10 group"
                      >
            ...
            <div className={`glass-panel flex-1 rounded-[4rem] flex flex-col items-center justify-center text-center p-12 border-dashed border-white/5 transition-all duration-1000 ${loading ? 'opacity-10 blur-2xl' : 'opacity-100'}`}>
              <div className="relative mb-10">
                <div className="w-32 h-32 rounded-full bg-white/5 flex items-center justify-center border border-white/5 relative">
                  <svg className={`w-12 h-12 text-orange-500 transition-all duration-700 ${loading ? 'animate-spin opacity-100' : 'opacity-20'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h3 className="text-xl font-bangers tracking-[0.3em] text-gray-800 uppercase mb-3">SYSTEM IDLE</h3>
              <p className="text-gray-800 max-w-xs text-[9px] uppercase tracking-[0.3em] font-black leading-relaxed opacity-40">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {history.slice(1, 5).map((h) => (
                  <button 
                    ...
                    className="flex flex-col gap-3 glass-panel p-3 rounded-[2rem] hover:bg-white/10 transition-all group border-white/5"
                  >
                    <div className="aspect-square w-full rounded-[1.2rem] bg-black/40 overflow-hidden border border-white/5">
                      {h.caricatureUrl ? <img src={h.caricatureUrl} className="w-full h-full object-cover grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500" /> : <div className="w-full h-full flex items-center justify-center text-[9px] text-gray-800 font-bold uppercase tracking-widest">LOG</div>}
                    </div>
                    <p className="text-[9px] font-black uppercase truncate text-gray-700 group-hover:text-white transition-colors px-1">{h.settings.targetName}</p>
                  </button>
                ))}
              </div>
            </div>
